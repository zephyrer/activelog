...
//	ASSERT_VALID(this);  // removed for speed
	...
	// warning C6387: 'argument 1' might be '0': this does not adhere to the specification for the function 'CallWindowProcW': Lines: 90, 91
...
HookWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	if (msg==WM_NCDESTROY) {
		...
		// pass to msg hook
		//  warning C6011: Dereferencing NULL pointer 'pSubclassWnd': Lines: 146, 147, 148, 149, 150, 151, 154, 155, 157, 158, 168
...
void CSubclassWndMap::Add(HWND hwnd, CSubclassWnd* pSubclassWnd)
{
	...
	if (pSubclassWnd->m_pNext==NULL) {
		...
		// SetWindowLongPtr function supersedes the SetWindowLong function. To write code that 
		// is compatible with both 32-bit and 64-bit versions of Microsoft Windows, 
		// use SetWindowLongPtr.
		// As well new style as described in Platform SDK:Rules for Using Pointers
		//
		// warning C6387: 'argument 1' might be '0': this does not adhere to the specification for the function 'SetWindowLongW': Lines: 205, 208, 209, 211, 218
		pSubclassWnd->m_pOldWndProc = 
			(WNDPROC)(LONG_PTR)::SetWindowLongPtr(
				hwnd, 
				GWLP_WNDPROC, 
				(LONG)(LONG_PTR)HookWndProc);
	...
	if (pHook==pUnHook) {
		...
		if (pHook->m_pNext)
			...
			SetWindowLongPtr(hwnd, 
						  GWLP_WNDPROC, 
						  (LONG)(LONG_PTR)pHook->m_pOldWndProc);
		}
	} else {
		// Hook to remove is in the middle: just remove from linked list
		// warning C6011: Dereferencing NULL pointer 'pHook': Lines: 236, 237, 239, 240, 241, 254
